...
class ContainerManager:
    def __init__(self, world):
        self.world = world
        self.config_path = robot_config.CONTAINER_CONFIG_PATH
    ...
    def respawn(self):
        ...
        if prim_utils.is_prim_path_valid(robot_config.CONTAINER_PRIM_PATH):
            prim_utils.delete_prim(robot_config.CONTAINER_PRIM_PATH)
        ...
        if prim_utils.is_prim_path_valid(robot_config.CONTAINER_POINT_PATH):
            point_prim = XFormPrim(robot_config.CONTAINER_POINT_PATH)
            spawn_pos, spawn_rot = point_prim.get_world_pose()

        # Spawn Container
        usd_path = os.path.join(robot_config.BASE_DIR, rel_path)
        add_reference_to_stage(usd_path=usd_path, prim_path=robot_config.CONTAINER_PRIM_PATH)
        
        self.container_prim = XFormPrim(
            prim_path=robot_config.CONTAINER_PRIM_PATH,
            name=name,
            position=spawn_pos,
